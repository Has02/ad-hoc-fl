        ...
        if idx == 0:
            for k in global_weights.keys():
                if (
                    ...
                    continue
                coeffs[k] = torch.exp(
                    torch.mul(-sigma, cos(global_weights[k], w_local[k]))
                )
        else:
            for k in global_weights.keys():
                if (
                ...
                coeffs[k] = torch.add(
                    coeffs[k],
                    torch.exp(torch.mul(-sigma, cos(global_weights[k], w_local[k]))),
                )
    return coeffs
...
def aggregate_cos(sigma, global_weights, local_weights):
    cos = torch.nn.CosineSimilarity(dim=0, eps=1e-6)
    coeffs = compute_cos_coeffs(
        sigma=sigma, global_weights=global_weights, local_weights=local_weights
    )
    w_glob = None
    for idx, w_local in enumerate(local_weights):
        if idx == 0:
            ...
            for k in global_weights.keys():
                if (
                    ...
                    w_glob[k] = torch.add(
                        w_glob[k], torch.mul(w_local[k], beta.to(preferred_device))
                    )  # FedMobility
...
def aggregate_avg(local_weights, drop_list=None):
    w_glob = None
    for idx, w_local in enumerate(local_weights):
        if idx == 0:
            w_glob = copy.deepcopy(w_local)
