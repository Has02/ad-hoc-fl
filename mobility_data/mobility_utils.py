...
def filter_devices(df, top_devices=100):
    ...
    id_occurrence = {
        k: v
        for k, v in sorted(
            id_occurrence.items(), key=lambda item: item[1], reverse=True
        )
    }
...
def check_duplicate_aps(mydict, all=False):
    ...
    for k in flipped.keys():
        ...
        if len(flipped[k]) > 1:
            flipped[k] = [
                np.random.choice(flipped[k])
            ]  # probabilities not given => samples assumes a uniform distribution over all entries
...
def distance_to_ap(long, lat, ap_df, AP=None):
    ...
    for k in aps_coords.keys():
        if AP != None and k == AP:
            ...
            dist = np.sqrt(
                (long - ap_long) * (long - ap_long) + (lat - ap_lat) * (lat - ap_lat)
            )
            ap_distances[k] = dist
            ap_name = [AP]
            break
...
def create_mobility_data(
    ...
    fname = (
        f"{mtype}_{number_devices}dev_{number_aps}ap_{connection}_{distance}m_s{seed}"
    )
    print(fname)
