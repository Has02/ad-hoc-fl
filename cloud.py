...
class DeviceHandler(threading.Thread):
    ...
    def run(self):
        ...
        self.connection = connect(
            ip=self.dev_ip, port=self.dev_port, verbose=self.verbose
        )
        ...
        if not self.simulation:
            send_file(
                connection=self.connection,
                target=self.dev_name,
                target_ip=self.dev_ip,
                target_port=self.dev_port,
                target_usr=self.dev_usr,
                target_pwd=self.dev_pwd,
                target_path=self.dev_path,
                filename=self.dev_model_filename,
...
class Cloud:
    ...
    def federated_learning(self):
        ...
        torch.save(
            net_glob.state_dict(), path.join(cloud_path, f"global_weights_0.pth")
        )
        ...
        if hierarchical:
            ...
            for ap_name in aplist:
                ...
                torch.save(
                    myaps[ap_name].to(torch.device(preferred_device)).state_dict(),
        ...
        if mobility:
            ...
            with open(
                f"{prefix}/{mtype}_{mobility_devices}dev_{aps}ap_wifi_lte_100m_s{seed}.pkl",
                "rb",
            ) as f:
                print(
            ...
            print(f"{mtype} {mobility_devices}dev")
            with open(
                f"{prefix}/{mtype}_{mobility_devices}dev_{aps}ap_wifi_lte_100m_s{seed}_nonmobility.pkl",
                "rb",
            ) as f:
                print(
        ...
        for comm_round in myrange:
                    ...
                    if hierarchical:
                        for ap_name in aplist:
                            os.remove(
                                path.join(
                                    cloud_path,
                                    f"{ap_name}_weights_{comm_round - 2}.pth",
                                )
                            )
                    os.remove(
            ...
            with open(self.dev_cfg, "r") as f:
                ...
                if len(available_devices) == 0:
                    print(
                        f"[!] Skipping communication round {comm_round} for t={t[t_idx]}"
                    )
                    ...
                    if hierarchical:
                        if (comm_round + 1) % k2 == 0:
                            ...
                            if len(ap_weights) > 0:
                                if cosine:
                                    ...
                                    w_glob = aggregate_cos(
                                        sigma=sigma,
                                        global_weights=cloud_w,
                                ...
                                torch.save(
                                    w_glob,
                                    path.join(
                                        cloud_path, f"global_weights_{comm_round}.pth"
                                    ),
                                )
                                print(
                                    f"Aggregated in {comm_round}: {trained_until_now}"
                                )
                                trained_until_now = []
                            else:
                        ...
                        torch.save(
                            net_glob.to(torch.device(preferred_device)).state_dict(),
                ...
                if simulation and mobility:
                    if ap_option == "hierfavg":
                        ...
                        if len(next_available_devices) > 0:
                            print(
                                f"Training only {len(next_available_devices)} devices in comm round {comm_round}"
                            )
                        ...
                        for i in range(1, k2 + 1):
                            ...
                            for d in mobility_data_filt[t[t_idx + i]]:
                                if d["internet_speed"] >= 0:
                                    if (
                                        d["device_idx"] in available_devices_idx
                                        and d["device_idx"]
                                        not in next_available_devices_idx
                                    ):
                                        next_available_devices.append(d)
                                        next_available_devices_idx.append(
                                            d["device_idx"]
                                        )

                            print(
                                f"\t Looking at future time idx={t_idx + i}; found {len(next_available_devices_idx)-prev_len} new devices"
                            )
                        if len(next_available_devices) > 0:
                            print(
                                f"Training only {len(next_available_devices)} devices in comm round {comm_round}"
                ...
                for mydict_key in avail_devs_idx_dict.keys():
                    ...
                    for idx, i in enumerate(avail_devs_idx_dict[mydict_key]):
                        ...
                        if hierarchical:
                            for d in available_devices:
                                if d["device_idx"] == i:
                                    ...
                                    if os.path.exists(
                                        ...
                                        torch.save(
                                            myaps[d["AP_name"][0]].state_dict(),
                                            path.join(cloud_path, dev_model_filename),
                                        )
                                        with open(
                                            f"logs/aps_{experiment}.csv", "a+"
                                        ) as logger:
                                            logger.write(
                                                f"{comm_round},{i},{d['AP_name'][0]},{d['device_type']},"
                            ...
                            for d in available_devices:
                                if d["device_idx"] == i:
                                    if not simulation:
                                        net_local = copy.deepcopy(net_glob).to(
                                            torch.device(dev_cuda_name)
                                        )
                                    else:
                        ...
                        device_info = (
                            f"{dev_type};{i};{data_iid};{num_users};"
                            f"{num_workers};{local_testing};{dev_cuda_name};{simulation};"
                            f"{verbose};{seed}"
                        )
                        data = (
                            f"{dev_model_name};{dev_model_filename};{local_epoch};"
                            f"{dev_train_batch_size};{test_batch_size};{learning_rate};"
                            f"{momentum};{loss_func_name};{dataset_name}"
                        )
                        logs = (
                            f"{log_all};{log_file};{log_acc};{log_loss};{log_train_time};{log_comm_time};{log_power};"
                            f"{log_temp}"
                        )
                        ...
                        device_handler_list.append(
                            DeviceHandler(
                                ...
                                dev_pwd=dev_pwd,
                                dev_path=dev_path,
                                dev_model_filename=dev_model_filename,
                    ...
                    print(
                        f"Started all clients - {len(avail_devs_idx_dict[mydict_key])}"
                    )
                    for idx, i in enumerate(avail_devs_idx_dict[mydict_key]):
            ...
            for d in mobility_data_filt[t[t_idx]]:
                if ap_option == "hierfavg":
                    ...
                    condition = devices_last_seen[d["device_idx"]][
                        "aggregated"
                    ] is False and devices_last_seen[d["device_idx"]][
                        "comm_round"
                    ] >= comm_round - (
                        comm_round % k2
                    )
            ...
            print(
                f"{len(next_available_devices)} devices will aggregate in comm round {comm_round}"
            )
            if len(next_available_devices) > 0:
                at_least_one = True
            else:
                    ...
                    if (comm_round + 1) % k2 == 0:
                        ...
                        for ap_name in aplist:
                            torch.save(
                                ...
                                .state_dict(),
                                path.join(
                                    cloud_path, f"{ap_name}_weights_{comm_round}.pth"
                                ),
                            )
                        print(f"[!] No edge aggregation this communication round!")
                        torch.save(
                ...
                if hierarchical:
                    ...
                    for elem in next_available_devices:
                        if ap_option == "hierfavg":
                            ...
                            condition = devices_last_seen[elem["device_idx"]][
                                "aggregated"
                            ] is False and devices_last_seen[elem["device_idx"]][
                                "comm_round"
                            ] >= comm_round - (
                                comm_round % k2
                            )
                        if condition:
                            if (
                                elem["AP_name"][0] not in trained_until_now
                                and ap_option != "hierfavg"
                            ):
                            ...
                            w_local = net_local.to(
                                torch.device(preferred_device)
                            ).state_dict()
                            if ap_option != "hierfavg":
                                local_weights[elem["AP_name"][0]].append(w_local)
                                devices_last_seen[elem["device_idx"]][
                                    "aggregated"
                                ] = True
                            else:
                                local_weights[elem["AP_name"][0]].append(w_local)
        ...
        with open(self.dev_cfg, "r") as f:
            ...
            for i in range(num_devices):
                ...
                dev_model_filename = f"dev_{i}.pth"
                torch.save(
                    net_local.state_dict(), path.join(cloud_path, dev_model_filename)
                )
                ...
                data = (
                    f"{model_name};{dev_model_filename};{local_epoch};{train_batch_size};"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cloud_cfg",
        type=str,
        default="configs/cloud_cfg_exp1.json",
        help="Cloud configuration file name",
    )
    parser.add_argument(
        "--dev_cfg",
        type=str,
        default="configs/dev_cfg_exp1.json",
