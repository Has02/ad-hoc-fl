...
class Device:
    def __init__(self, connection, dev_ip_port, exp, r, dev_idx):
        ...
        (
            self.dataset_train,
            self.dict_users_train,
            self.dataset_test,
            self.dict_users_test,
        ) = (None, None, None, None)
        ...
        self.local_testing, self.cuda_name, self.simulation, self.verbose = (
            None,
            None,
            None,
            None,
        ...
        self.local_epochs, self.train_batch_size, self.test_batch_size = (
            None,
            None,
            None,
        )
        self.learning_rate, self.momentum = None, None
        self.loss_func, self.loss_func_name, self.model_path, self.dataset_name = (
            None,
            None,
            None,
            None,
        ...
        self.log_acc, self.log_loss, self.log_train_time, self.log_comm_time = (
            True,
            True,
            True,
            True,
    ...
    def run(self):
        ...
        assert msg != None, "[!] Received no input from client"
        ...
        train_loss, train_acc, test_loss, test_acc, train_time = local_training(
            ...
            log_train_time=self.log_train_time,
            verbose=self.verbose,
            data_iid=self.data_iid,
            seed=self.seed,
        )
...
def run(stop, device_type, measurement_rate, dev_idx, log_power, log_temp, exp, r):
    if device_type != "nano":
        ...
        if log_power and log_temp:
            ...
            with open(
                path.join("logs", f"device{dev_idx}_pow_temp_log_exp{exp}_run{r}.csv"),
                "w",
            ) as logger:
        ...
        while True:
            ...
            if log_power and log_temp:
                with open(
                    path.join(
                        "logs", f"device{dev_idx}_pow_temp_log_exp{exp}_run{r}.csv"
                    ),
                    "a+",
                ) as logger:
            ...
            time.sleep(max(0.0, measurement_rate - (time.time() - loop_start)))
            if stop():
                break
    else:
        if not os.path.exists(f"logs"):
        ...
        os.system(
            "echo password | sudo -S tegrastats --interval 1000 --logfile tegrastats.log &"
        )
        time.sleep(2)
        print(f"[+] Recording power and temperature...")
        file = "tegrastats.log"
        with open(
            path.join("logs", f"device{dev_idx}_pow_temp_log_exp{exp}_run{r}.csv"), "w"
        ) as logger:
            logger.write(
                f"Timestamp,RAM,SWAP,CPU1 %,CPU1_FREQ MHz,CPU2 %,CPU2_FREQ MHz,CPU3 %,CPU3_FREQ MHz,CPU4 %,CPU4_FREQ MHz,CPU_TEMP C,GPU %,GPU_FREQ MHz,GPU_TEMP C,TOTAL_TEMP,TOTAL_POWER mW,TOTAL_AVG mW,GPU_POW mW,GPU_AVG_POW mW,CPU_POW mW,CPU_AVG_POW mW\n"
            )
        while True:
            loop_start = time.time()
            ...
            with open(
                ...
                gpu_freq = (
                    (str(last).split("GR3D_FREQ "))[1].split(" VIC_FREQ")[0]
                ).split("@")[1]
                gpu_temp = (str(last).split("GPU@"))[1].split("C AO@")[0]
                thermal = (str(last).split("thermal@"))[1].split("C POM_5V")[0]

                power = (
                    (str(last).split("POM_5V_IN "))[1].split(" POM_5V_GPU")[0]
                ).split("/")[0]
                avg = (
                    (str(last).split("POM_5V_IN "))[1].split(" POM_5V_GPU")[0]
                ).split("/")[1]
                gpu_power = (
                    (str(last).split("POM_5V_GPU "))[1].split(" POM_5V_CPU")[0]
                ).split("/")[0]
                gpu_avg = (
                    (str(last).split("POM_5V_GPU "))[1].split(" POM_5V_CPU")[0]
                ).split("/")[1]
                cpu_power = (str(last).split("POM_5V_CPU "))[1].split("/")[0]
                cpu_avg = ((str(last).split("POM_5V_CPU "))[1].split("\\")[0]).split(
                    "/"
                )[1]
                logger.write(
                    f"{time.time()},{ram},{swap},{cpu_val[0]},{cpu_freq[0]},{cpu_val[1]},{cpu_freq[1]},"
...
def start_device(
    host, port, device_type, measurement_rate, dev_idx, log_power, log_temp, exp, r
):
    soc = socket.socket()
    ...
    while True:
        try:
            ...
            soc.close()
            print(
                f"[!] TIMEOUT Socket Closed Because no Connections Received.\n[!] ERROR: {e}\n"
            )
            if log_power or log_temp:
                stop_threads = True
                t1.join()
...
parser.add_argument(
    "--measurement_rate", type=float, default=1, help="Measurement rate in seconds"
)
parser.add_argument(
    "--log_file", type=str, default="log_file.csv", help="Log file name"
)
